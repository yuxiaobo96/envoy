...
#else
  char test_udsdir[] = "c:\\Windows\\TEMP\\envoy_test_uds.XXXXXX";
#endif
...
#else
  char test_tmpdir[] = "c:\\Windows\\TEMP\\envoy_test_tmp.XXXXXX";
#endif
...
#if defined(WIN32)
  // Substitute NUL for /dev/nul.
  const std::regex dev_null_regex("\\/dev\\/null");
  out_json_string = std::regex_replace(out_json_string, dev_null_regex, "NUL");
#endif
