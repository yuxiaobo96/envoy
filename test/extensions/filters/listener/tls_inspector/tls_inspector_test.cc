...
TEST_F(TlsInspectorTest, SniRegistered) {
  ...
  EXPECT_CALL(os_sys_calls_, recv(42, _, _, MSG_PEEK))
      .WillOnce(Invoke(
          [&client_hello](SOCKET_FD, void* buffer, size_t length, int) -> Api::SysCallSizeResult {
...
TEST_F(TlsInspectorTest, ClientHelloTooBig) {
  ...
  init();
  EXPECT_CALL(os_sys_calls_, recv(42, _, _, MSG_PEEK))
      .WillOnce(Invoke([=, &client_hello](SOCKET_FD, void* buffer, size_t length,
...
TEST_F(TlsInspectorTest, NotSsl) {
  ...
  EXPECT_CALL(os_sys_calls_, recv(42, _, _, MSG_PEEK))
      .WillOnce(
          Invoke([&data](SOCKET_FD, void* buffer, size_t length, int) -> Api::SysCallSizeResult {
