...
function bazel_binary_build($type) {
  echo "Building..."
  bazel  $env:BAZEL_BASE_OPTIONS.Split(" ") build $env:BAZEL_BUILD_OPTIONS.Split(" ") -c $type "//source/exe:envoy-static" --test_tag_filters=-skip_on_windows
  $exit = $LASTEXITCODE
  if ($exit -ne 0) {
    exit $exit
  }
...
function bazel_test($type, $test) {
  if ($test) {
    ...
    bazel $env:BAZEL_BASE_OPTIONS.Split(" ") test $env:BAZEL_BUILD_OPTIONS.Split(" ") -c $type "//test/..." --test_tag_filters=-skip_on_windows --build_tests_only --test_summary=terse --test_output=errors -- "-//test/extensions/..." "-//test/integration/..."
  }
  exit $LASTEXITCODE
}
